...
class fillIdentifyNumberViewController: UIViewController, UITextFieldDelegate {
    ...
    func registerBtn(sender : UIButton){
        ...
        var totalStr :String = serviceurl + "login?imsi=460011418603054&versionNo=0.1"
        ...
        var e : NSError?
        var josnR : NSDictionary? = NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.MutableContainers, error: nil) as? NSDictionary
            println(josnR)
            if josnR == nil{
                 println(e.description)
            }else{
               if josnR!["res"].integerValue == 0{
                tokenNo = josnR!["msg"] as? String
                println(tokenNo!)
                }
               else{
                /**进行注册*/
