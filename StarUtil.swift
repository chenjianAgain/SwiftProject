...
class StarUtil: NSObject {
    class func showDriverStar(grade : Float,view: UIView){
        /*取得整数部分*/
        var fullStar  = floor(grade)
        if CGFloat(fullStar) == CGFloat(grade){
            if CGFloat(fullStar) == 0.0{
                for var z = 0; z < 5 ; z++ {
                    var emptyStarImg : UIImageView = UIImageView(frame: CGRectMake(CGFloat(z) * 14, 0, 14, view.frame.size.height))
            ...
            if Int(fullStar) < 5 && Int(fullStar) != 0{
